...
#
# Comparación t entre grupos, misma función que Tabla 2. Justifica
# la exclusión de HADS-ansiedad como covariable de sensibilidad en
# H1/H2 por riesgo de sobrecontrol (ver Método).
# ============================================================

HADS_VARS = {"hads_ansiedad_total": "HADS ansiedad", "hads_depresion_total": "HADS depresión"}

filas_hads = []
for var, label in HADS_VARS.items():
    r = comparar_grupos(jovenes[var], mayores[var])
    filas_hads.append({
        "Medida": label,
        "Jóvenes N": r["n_joven"], "Jóvenes M": round(r["media_joven"], 2), "Jóvenes SD": round(r["sd_joven"], 2),
        "Mayores N": r["n_mayor"], "Mayores M": round(r["media_mayor"], 2), "Mayores SD": round(r["sd_mayor"], 2),
...
tabla_a1 = pd.DataFrame(filas_hads)
tabla_a1.to_csv(OUTPUT_DIR / "tablaA1_hads_por_grupo.csv", index=False)


# ============================================================
# TABLA A3 — CORRELACIONES HADS x MEDIDAS BART (N=74)
#
# Justifica que HADS-depresión no se incluyó como covariable
# adicional (no correlaciona con ninguna medida de BART). Se
# reportan Pearson y Spearman porque ninguna de las cuatro
# variables involucradas cumple normalidad (Shapiro-Wilk, p<.05
# en las cuatro).
# ============================================================

filas_hads_bart = []
for hads_var, hads_label in HADS_VARS.items():
    for outcome in OUTCOMES:
        r, p_pearson = stats.pearsonr(base[hads_var], base[outcome])
        rho, p_spearman = stats.spearmanr(base[hads_var], base[outcome])
        filas_hads_bart.append({
            "Predictor": hads_label,
            "Medida BART": OUTCOME_LABELS[outcome],
            "N": len(base),
            "r": round(r, 3), "p (Pearson)": round(p_pearson, 3),
            "rho": round(rho, 3), "p (Spearman)": round(p_spearman, 3),
        })

tabla_a3 = pd.DataFrame(filas_hads_bart)
tabla_a3.to_csv(OUTPUT_DIR / "tablaA3_correlaciones_hads_bart.csv", index=False)

print("\nTabla A1 (HADS por grupo):")
print(tabla_a1.to_string(index=False))

print("\nTabla A3 (correlaciones HADS x BART):")
