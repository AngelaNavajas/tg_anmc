# =====================================================================
# 02_H2_mediacion_PROCESS_v2.R
#
# H2 — Mediación simple con PROCESS v4.3.1
#
...
# Muestra:
#   Base maestra = N 74
#   H2 Modelo A  = N 73 (excluido solo ID 4)
#
# Bootstrap: percentil, 10.000 remuestras, seed = 33333
# =====================================================================

# =====================================================================
# 1. CARGAR PROCESS v4.3.1 Y PAQUETE mediation
# =====================================================================
...
PROJECT_ROOT <- "/Users/angelanavajas/Desktop/anmc"

DATA_PATH <- file.path(PROJECT_ROOT, "datos", "limpios", "base_maestra_final.csv")

OUTPUT_DIR <- file.path(PROJECT_ROOT, "outputs", "H2_PROCESS_v2")
...
#
# Ya NO se usa una lista de IDs hardcodeada. Se filtra directamente
# por la columna incluido_h2_speed, construida en
# 20_crear_processing_speed_final.py y propagada a base_maestra_final.csv.
# Esto evita que el criterio de exclusión quede duplicado (y
# potencialmente desincronizado) entre el script de Python y este
# script de R.
# =====================================================================
...
# =====================================================================
# 6. FUNCIONES DE EXTRACCIÓN (ahora con LLCI/ULCI de a, b, c, c')
# =====================================================================
...
extraer_resultados_process <- function(salida, modelo, outcome, tipo_modelo,
  ...
  a_vals      <- extraer_numeros(lineas_x[1])  # modelo del mediador
  cprime_vals <- extraer_numeros(lineas_x[2])  # modelo del outcome (c')
  c_vals      <- extraer_numeros(lineas_x[3])  # total effect model (c)
  ...
  # coeff, se, t, p, LLCI, ULCI (6 números) -> se guardan TODOS ahora
  paths <- data.frame(
...
# =====================================================================
# 7. EJECUTAR MODELO PROCESS (genérico: sirve para X, M y datos distintos)
# =====================================================================

ejecutar_modelo <- function(modelo, datos, x_var, m_var, outcome,
                             tipo_modelo, covariables = NULL) {
...
M1 <- ejecutar_modelo("M1_adjusted_pumps", datos_h2, "grupo_code",
                       "processing_speed", "adjusted_pumps", "Principal")

M2 <- ejecutar_modelo("M2_total_money", datos_h2, "grupo_code",
                       "processing_speed", "total_money", "Secundario")

M3 <- ejecutar_modelo("M3_total_explosions", datos_h2, "grupo_code",
                       "processing_speed", "total_explosions",
                       "Secundario_exploratorio")

# Covariables de sensibilidad: sexo + edad_std_intragrupo
# (se retira anos_educacion por decisión de la reunión)
COVARIABLES <- c("genero_code", "edad_std_intragrupo")

M1_S <- ejecutar_modelo("M1S_adjusted_pumps_controles", datos_h2, "grupo_code",
                         "processing_speed", "adjusted_pumps", "Sensibilidad",
                         covariables = COVARIABLES)

M2_S <- ejecutar_modelo("M2S_total_money_controles", datos_h2, "grupo_code",
                         "processing_speed", "total_money", "Sensibilidad",
                         covariables = COVARIABLES)

M3_S <- ejecutar_modelo("M3S_total_explosions_controles", datos_h2, "grupo_code",
                         "processing_speed", "total_explosions",
...
#    (grupo etario -> HADS ansiedad -> explosiones totales)
#    Usa la base completa (N=74): HADS no depende de exactitud CRT.
# =====================================================================
...
# =====================================================================
# 10. TABLAS GENERALES (Modelo A + Modelo B)
# =====================================================================
...
write.csv(tabla_paths, file.path(OUTPUT_DIR, "H2_tabla_paths_PROCESS_v2.csv"),
          row.names = FALSE, fileEncoding = "UTF-8")
write.csv(tabla_indirectos, file.path(OUTPUT_DIR, "H2_tabla_efectos_indirectos_PROCESS_v2.csv"),
          row.names = FALSE, fileEncoding = "UTF-8")

cat("\n\nTABLA DE PATHS (con p-valores e IC de a, b, c, c'):\n")
...
# =====================================================================
# 11. P-VALOR EXPLÍCITO DEL EFECTO INDIRECTO (paquete `mediation`)
#
# PROCESS no reporta p-valor para el efecto indirecto (a x b) porque
# su distribución muestral no es normal — solo reporta si el IC
# bootstrap excluye 0. Si el mentor/comité pide un p-valor puntual,
# el paquete `mediation` (Tingley et al.) sí lo reporta (columna
# p-value de ACME), usando simulación quasi-bayesiana en vez de
# bootstrap percentil. Ambos son válidos; reportar cuál se usó.
# =====================================================================
...
correr_mediation <- function(datos, x_var, m_var, y_var, covariables = NULL) {
  ...
  # FIX (v2): environment(formula) <- globalenv() NO alcanza — eso solo
  # cambia dónde se buscan variables DENTRO de la fórmula, no crea el
  # objeto "formula_m" en sí. lm() guarda su llamada literal
  # (formula = formula_m, data = datos) con esos nombres de símbolo sin
  # resolver, y mediate() la vuelve a evaluar en cada remuestra bootstrap.
  # Si "formula_m" y "datos" no existen como variables reales en el
  # entorno donde mediate() reevalúa, falla. Solución: asignarlas como
  # variables globales con nombre fijo antes de ajustar los modelos.
...
# --------------------------------------------------------------
# Especificación de los 7 modelos (igual que en las secciones 8-9)
# para correr mediation() sobre todos y obtener el p-valor de ACME
# en cada uno, no solo en el principal.
# --------------------------------------------------------------
