...
#
# Los modelos se estiman con errores estándar HC3 (heterocedasticidad
# consistente), dado un resultado significativo de la prueba de Levene
# para el mediador.
#
# Muestra:
#   Base maestra = N 74
#   Muestra de análisis = N 73 (excluido ID 4)
#
...
# =====================================================================
# 9. EFECTO TOTAL (c) SIN COVARIABLES
#
...
# iguales. Se calcula sobre la base completa (N = 74), no sobre la
# muestra restringida de processing_speed (N = 73), dado que esta
# comparación no involucra el mediador.
# =====================================================================

datos_completa <- base
datos_completa$grupo_code <- ifelse(datos_completa$grupo == "joven", 0,
                                    ifelse(datos_completa$grupo == "mayor", 1, NA))
...
efecto_total <- rbind(
  calcular_efecto_total_ols("adjusted_pumps", datos_completa),
  calcular_efecto_total_ols("total_money", datos_completa),
  calcular_efecto_total_ols("total_explosions", datos_completa)
)
...
cat("N muestra de análisis:", nrow(datos_h2), "\n")
